...
static unsigned int	functions(unsigned int x, unsigned int y,
				unsigned int z, char func)
{
	if (func == 'F')
		return ((x & y) | ((~x) & z));
	else if (func == 'G')
...
static void	execute_ops(unsigned int *abcd, unsigned int *x, const char *op)
{
...
t_wrap	md5_process_message(t_wrap msg, size_t msg_bits, t_wrap abcd)
{
	...
	while (i < msg_bits / (32 * 16))
	{
		...
		while (j < 64)
			execute_ops(abcd.i, x, g_operations[j++]);
		abcd.i[0] += tmp_abcd[0];
		abcd.i[1] += tmp_abcd[1];
		abcd.i[2] += tmp_abcd[2];
		abcd.i[3] += tmp_abcd[3];
